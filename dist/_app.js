...
async function readFileJson(filename, encoding) {
...
async function main() {
    ...
    const dataFolder = ((dataFolderQuestion !== "" ? dataFolderQuestion : "C:\\Program Files (x86)\\Steam\\steamapps\\common\\Book of Hours") +
    ...
    loader.setDataItems([
        ...
        tomes,
        prototypes,
    ].flatMap(files => files.elements.map((element) => ({
        id: element.id,
    ...
    while (true) {
        ...
        switch (parts[0]) {
            ...
            case "list": {
                switch (parts[1]) {
                    case "aspects": {
                        console.log([...loader.getAllAspects()].sort().join(", "));
                        continue;
                    }
            ...
            case "search": {
                switch (parts[1]) {
                    case "items": {
                        try {
                            const args = JSON.parse(parts.slice(2).join(" "));
                    ...
                    case "recipes": {
                        try {
                            const arg = JSON.parse(parts.slice(2).join(" "));
